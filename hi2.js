(() => {
  ...
  class rocket2 {
    constructor(speed, angleXY, angleZ, launchBtn, maxHeightSpan, rangeSpan){
      ...
      this.angleXY = angleXYInput
      this.angleZ = angleZInput
  ...
  a = new rocket2(speedInput.value, angleXYInput.value, angleZInput.value, launchBtn, maxHeightSpan, rangeSpan);
  ...
  function createRocket() {
    ...
    const bodyMaterial = new MeshPhongMaterial({ color: 'blue' });
    ...
    const coneMaterial = new MeshPhongMaterial({ color: 'skyblue' });
  ...
  const windDirInput = document.getElementById('windDir2');
  const windSpeedInput = document.getElementById('windSpeed2')
    ...
    if (rocketBody && launched) {
      if (!landed) {
        ...
        if (rocketBody.position.y <= 1.3 && !touchedGround && isMaxheight) {
          ...
          const markerMaterial = new THREE.MeshBasicMaterial({ color: 0x0000ff }); // 빨간색
